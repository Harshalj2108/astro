...
def calculate_venus_sidereal_longitude(date, ephe_path='ephe'):
    """
    Calculate the sidereal longitude of the venus for a given date
    ...
    -------
    float
        venus's sidereal longitude in degrees.
    """
    # Set ephemeris path
    ...
    try:
        result = swe.calc_ut(jd, swe.VENUS, flags)
        if isinstance(result, int):
            raise RuntimeError(f"swe.calc_ut failed with error code: {result}")
        pos, _ = result  # pos is the tuple (longitude, latitude, ...)
        venus_sidereal_long = pos[0]  # Sidereal longitude in degrees
    except Exception as e:
        raise RuntimeError(f"Error calculating venus's position: {e}")

    return venus_sidereal_long
