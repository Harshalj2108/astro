...
def calculate_mars_sidereal_longitude(date, ephe_path='ephe'):
    """
    Calculate the sidereal longitude of the mars for a given date
    ...
    -------
    float
        mars's sidereal longitude in degrees.
    """
    # Set ephemeris path
    ...
    try:
        result = swe.calc_ut(jd, swe.MARS, flags)
        if isinstance(result, int):
            raise RuntimeError(f"swe.calc_ut failed with error code: {result}")
        pos, _ = result  # pos is the tuple (longitude, latitude, ...)
        mars_sidereal_long = pos[0]  # Sidereal longitude in degrees
    except Exception as e:
        raise RuntimeError(f"Error calculating mars's position: {e}")

    return mars_sidereal_long
