...
def calculate_uranus_sidereal_longitude(date, ephe_path='ephe'):
    """
    Calculate the sidereal longitude of the uranus for a given date
    ...
    -------
    float
        uranus's sidereal longitude in degrees.
    """
    # Set ephemeris path
    ...
    try:
        result = swe.calc_ut(jd, swe.URANUS, flags)
        if isinstance(result, int):
            raise RuntimeError(f"swe.calc_ut failed with error code: {result}")
        pos, _ = result  # pos is the tuple (longitude, latitude, ...)
        uranus_sidereal_long = pos[0]  # Sidereal longitude in degrees
    except Exception as e:
        raise RuntimeError(f"Error calculating uranus's position: {e}")

    return uranus_sidereal_long
