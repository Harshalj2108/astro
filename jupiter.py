...
def calculate_jupiter_sidereal_longitude(date, ephe_path='ephe'):
    """
    Calculate the sidereal longitude of the jupiter for a given date
    ...
    -------
    float
        jupiter's sidereal longitude in degrees.
    """
    # Set ephemeris path
    ...
    try:
        result = swe.calc_ut(jd, swe.JUPITER, flags)
        if isinstance(result, int):
            raise RuntimeError(f"swe.calc_ut failed with error code: {result}")
        pos, _ = result  # pos is the tuple (longitude, latitude, ...)
        jupiter_sidereal_long = pos[0]  # Sidereal longitude in degrees
    except Exception as e:
        raise RuntimeError(f"Error calculating jupiter's position: {e}")

    return jupiter_sidereal_long
