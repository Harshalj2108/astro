...
def calculate_rahu_sidereal_longitude(date, ephe_path='ephe'):
    """
    Calculate the sidereal longitude of Rahu (Moon's ascending node) for a given date
    ...
    -------
    float
        Rahu's sidereal longitude in degrees.
    """
    # Set ephemeris path
    ...
    # Calculate Rahu's position (true node)
    ...
    try:
        ...
        rahu_sidereal_long = pos[0]  # Sidereal longitude in degrees
    except Exception as e:
        raise RuntimeError(f"Error calculating Rahu's position: {e}")

    return rahu_sidereal_long
