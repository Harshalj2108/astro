...
def calculate_mercury_sidereal_longitude(date, ephe_path='ephe'):
    """
    Calculate the sidereal longitude of the mercury for a given date
    ...
    -------
    float
        mercury's sidereal longitude in degrees.
    """
    # Set ephemeris path
    ...
    try:
        result = swe.calc_ut(jd, swe.MERCURY, flags)
        if isinstance(result, int):
            raise RuntimeError(f"swe.calc_ut failed with error code: {result}")
        pos, _ = result  # pos is the tuple (longitude, latitude, ...)
        mercury_sidereal_long = pos[0]  # Sidereal longitude in degrees
    except Exception as e:
        raise RuntimeError(f"Error calculating mercury's position: {e}")

    return mercury_sidereal_long
